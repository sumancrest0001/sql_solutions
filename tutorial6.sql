...
SELECT DISTINCT stadium, COUNT(stadium) FROM game JOIN goal ON (matchid = id) 
GROUP BY stadium;
...
SELECT matchid, mdate, COUNT(matchid) FROM game JOIN goal ON (matchid = id) WHERE teamid = 'GER'
GROUP BY matchid, mdate;
